...
// Копируем массив resSerial в массив resParallel
...
void PrepareParallelCountingSort(int[] inputArray)
{
    ...
    // Создаем коллекцию(список), в которую будем отправлять результаты вычислений потоков
    var threadsParall = new List<Thread>(); 

    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        // Устанавливаем диапазоны для наших потоков
...
void CountingSortParallel(int[] inputArray, int[] counters, int offset, int startPos, int endPos)
{
    ...
    {
        // Пока один поток работает, следующий ждет
        lock (lock_object)
        {
...
// Функция сравнения двух масиивов, массив который получился при последовательной
// сортировке и массив который получился при параллельной сортировке
